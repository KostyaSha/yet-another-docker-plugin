...
import static com.github.kostyasha.yad.utils.HostAndPortChecker.create;
...
public class    PortUtilsTest {
    ...
    public void shouldConnectToServerSuccessfully() throws Exception {
        assertThat("Server is up and should connect", create(server.host(), server.port()).openSocket(), is(true));
    }

    @Test
    public void shouldNotConnectToUnusedPort() throws Exception {
        assertThat("Unused port should not be connectible", create("localhost", 0).openSocket(), is(false));
    }
    ...
    public void shouldWaitForPortAvailableUntilTimeout() throws Exception {
        ...
        assertThat("Unused port should not be connectible",
                create("localhost", 0).withRetries(RETRY_COUNT)
