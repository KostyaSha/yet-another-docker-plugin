...
public class DockerEnvironmentContributor extends EnvironmentContributor {
    ...
    public void buildEnvironmentFor(@Nonnull Run run, @Nonnull EnvVars envs, @Nonnull TaskListener listener)
        ...
        final DockerOptionalJobProperty dockerProperty = (DockerOptionalJobProperty) job.getProperty(DockerOptionalJobProperty.class);
        if (dockerProperty == null) {
            LOG.debug("[YAD] Yet Another Docker optional job property not found for {}", job.getFullName());
            return;
        }

        final Executor executor = run.getExecutor();
        if (isNull(executor) || !(executor.getOwner() instanceof DockerComputer)) {
            LOG.debug("[YAD] Run {} is not running on YADocker Computer", run);
            return;
        }

        if (isFalse(dockerProperty.getInjectVars())) {
            LOG.debug("[YAD] Variables injection is disabled for {}", run);
            return;
        }
